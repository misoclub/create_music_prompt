const INSTRUCTION_TEXT = `というスタイルを軸に曲を作りたいです。
スタイルはあくまで参考とし、歌詞に合わせてスタイルは変更してください。
...
const JAPAN_INSTRUCTION_TEXT = `というスタイルを軸に曲を作りたいです。
スタイルはあくまで参考とし、歌詞に合わせてスタイルは変更してください。
...
・Sunoに渡す曲のスタイルのプロンプト
・Sunoに渡すタグ付きの歌詞
・漢字を全部ひらがなにした歌詞。（英語はそのままでいいです）単語ごとにスペースを入れてください。
・どんな内容の歌詞かの簡単な説明
以上の5点を出力してください。
コピペしやすいように簡単な説明以外にはコピーボタンをつけてください。
毎回同じプロンプトで生成しているので、今回はあなたならではのランダム性を入れて歌詞を考えてください。
